...
import com.bbdog.demo.entity.IdEntity;
import com.bbdog.demo.entity.Page;
import com.bbdog.demo.mapper.BaseMapper;
import com.bbdog.demo.utils.format.ReflectionUtils;
import com.bbdog.demo.utils.UID;
...
public abstract class MyBatisDao<T extends IdEntity> extends MybatisSessionDaoSupport implements BaseMapper<T> {
	...
	public int delete(final T entity) {
		...
		if(logger.isDebugEnabled()){
			logger.debug("删除实体 "+entity.getClass().getName()+"id="+entity.getId());
		}
		BaseMapper<T> mapper = getBaseMapper();		
